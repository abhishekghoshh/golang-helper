...
func SockerServer() {
	host := flag.String("h", "0.0.0.0", "The IP address to use")
	portFlag := flag.Int("p", 8080, "The port to use.")
	flag.Parse()

	ip := net.ParseIP(*host)
	port := *portFlag
	// creating new socket with ip and port
...
func processNetSocket(rw *netSocket) {
	// Read request
