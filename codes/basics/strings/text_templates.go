...
Go offers built-in support for creating dynamic content or showing customized output
to the user with the text/template package. A sibling package named html/template provides
the same API but has additional security features and should be used for generating HTML.
*/
func TestTemplates() {
	...
	// for its actions. The {{.}} action is replaced by the value passed as a parameter to Execute.
	...
	// if/else provide conditional execution for templates. A value is considered false
	// if it's the default value of a type, such as 0, an empty string, nil pointer, etc.
