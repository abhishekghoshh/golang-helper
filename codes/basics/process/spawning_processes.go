...
func DoSpawningProcess() {
	...
	// its standard output. If there were no errors, dateOut will hold bytes with the date info.
	...
	// Output and other methods of Command will return *exec.Error
	// if there was a problem executing the command (e.g. wrong path),
	...
	// We omitted error checks in the above example, but you could use the usual if err != nil pattern for all of them.
	...
	// being able to just pass in one command-line string. If you want to spawn a full command with a string, you can use bash's -c option:
