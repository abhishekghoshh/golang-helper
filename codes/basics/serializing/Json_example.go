...
func TestJSONHandling() {
	...
	fmt.Println("serializing boolean type :", string(bolB))

	intB, _ := json.Marshal(1)
	fmt.Println("serializing int type :", string(intB))

	fltB, _ := json.Marshal(2.34)
	fmt.Println("serializing float type :", string(fltB))

	strB, _ := json.Marshal("gopher")
	fmt.Println("serializing string type :", string(strB))
	...
	fmt.Println("serializing slice type :", string(slcB))
	...
	fmt.Println("serializing map type :", string(mapB))
	...
	fmt.Println("serializing struct type without tags :", string(res1B))
	...
	fmt.Println("serializing struct type with json tags :", string(res2B))
	...
	fmt.Println("deserializing from byte data :", dat)
	...
	num := dat["num"].(float64)
	fmt.Println("getting value from desirialized map :", num)

	// Accessing nested data requires a series of conversions.
	strs := dat["strs"].([]interface{})
	str1 := strs[0].(string)
	fmt.Println("getting value from desirialized map :", str1)
	...
	fmt.Println("deserializing from string data :", res)
	fmt.Println("getting value from desirialized struct :", res.Fruits[0])

	// In the examples above we always used bytes and strings as intermediates between the data and JSON
	// representation on standard out. We can also stream JSON encodings directly to os.Writers like os.Stdout or even HTTP response bodies.
