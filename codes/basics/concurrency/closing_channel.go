...
func ClosingChannel() {
	...
	// the more value will be false if jobs has been closed and all values in the channel
	// have already been received. We use this to notify on done when we've worked all our jobs.
	go func(ch chan int) {
		for {
			j, more := <-ch
			if more {
	...
	// This sends 6 jobs to the worker over the jobs channel, then closes it.
	for j := 1; j <= 6; j++ {
		jobs <- j
	...
	// returning the zero value of the underlying type. The optional second return value is true
	...
	// The idea of closed channels leads naturally to range over channels.
}
