...
or a command-line tool to stop processing input if it receives a SIGINT. Here's how to handle signals in Go with channels.
*/
...
func DoSignals() {
	...
	// We could receive from sigs here in the main function, but let's see how this could also be done
	// in a separate goroutine, to demonstrate a more realistic scenario of graceful shutdown.
	...
	// (which the terminal shows as ^C) we can send a SIGINT signal, causing the program to print interrupt and then exit.
}
